...
## 2026-04-24 06:14 — v0.4.2 Session B — config migration + first-run MTGO detect + release  [batch]

**Type:** status

Session B landed as commit `308c89d`, tag `v0.4.2`, CI run 24875083409 green (2m31s), release run 24875174627 green. `DeepAnalysisAgentSetup.exe` (36,116,958 bytes), `DeepAnalysisAgent-0.4.2-full.nupkg` (35,881,950 bytes), and `RELEASES` attached to https://github.com/sentania-labs/deep-analysis-agent/releases/tag/v0.4.2. Three behaviour changes: (1) `load_config()` now detects any `mtgo.log_dir` containing `Users\Default\` or `users\default\` (case-insensitive — covers both backslash TOML literals) and rewrites it to the current LOCALAPPDATA-derived default via `save_config()`. New `_has_stale_default_user_path()` helper. Fixes 0.4.0 installs where SYSTEM-context wrote a `C:\Users\Default\...` path into the first on-disk config. Idempotent — second load is a no-op. Three new unit tests cover rewrite, idempotency, and lowercase detection (all use `monkeypatch.setattr(config_module, "save_config", ...)` to avoid disk writes). (2) `run_first_run_flow()` now calls `_resolve_mtgo_log_dir(config)` after successful registration: uses the default if it exists on disk, else prompts `tkinter.filedialog.askdirectory`, falls back to the default (with a warning) if the user cancels or tkinter is unavailable. Never blocks registration. (3) Version bumped 0.4.1 → 0.4.2 in `pyproject.toml`, `src/deep_analysis_agent/__init__.py`, and `first_run.CLIENT_VERSION`. 47 tests pass / 1 skip on Linux; ruff + mypy strict clean. PR-flip deferred to post-smoke confirmation per Riker's brief — Scott smokes the 0.4.1 → 0.4.2 auto-update on xps9530 in the morning, then Riker decides.
