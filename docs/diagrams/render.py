...
def render(excalidraw_path: Path, output_path: Path | None = None, scale: int = 2, max_width: int = 1920) -> Path:
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("ERROR: playwright not installed.", file=sys.stderr)
        print("Run: cd docs/diagrams && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd docs/diagrams && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
