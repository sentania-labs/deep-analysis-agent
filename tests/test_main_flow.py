...
async def test_handle_file_skips_when_version_blocked(
    ctx: tuple[AppConfig, DedupStore, _StubTray, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When version_blocked is set, _handle_file skips the upload entirely."""
    cfg, dedup, tray, sample = ctx
...
async def test_version_block_cleared_after_update_resumes_uploads(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """After version goes from blocked to acceptable, uploads resume.

    We run two heartbeat-loop passes sharing the same version_blocked event:
    pass 1 blocks (version too old), pass 2 unblocks (requirement lowered).
    """
    cfg = AppConfig()
    cfg.server.url = "https://example.test"
    cfg.agent.api_token = "tok"
    ...
    # --- Pass 2: version now acceptable → unblock ---
    stop2 = asyncio.Event()
    ...
    assert tray.states[-1] == "idle", "tray should return to idle after version unblock"
