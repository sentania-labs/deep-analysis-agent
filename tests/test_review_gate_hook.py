...
command and exit 2 blocks it. Agent issue #48: the hook resolved HEAD
from the harness cwd, which is always the PRIMARY checkout, so a commit
made in a ``git worktree`` could never satisfy it, and ``git -C <path>``
pushes were not matched at all.
...
# --- codex review round 1: forms that previously slipped past the parser ---
